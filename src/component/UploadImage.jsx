...
const UploadProfile = ({ register, data }) => {
  ...
  const captureImage = () => {
    ...
    if (canvas && video) {
      ...
      // Check clarity based on average brightness
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-100 to-gray-200">
      ...
      <button className="w-64 py-3 text-white bg-red-600 rounded-full hover:bg-red-700 text-lg font-medium shadow-lg transition" onClick={() => register(formData)}>SIGN UP</button>
    </div>
  );
};
